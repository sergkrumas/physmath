...
# AlgsLab von Sergei Krumas 

# НАХОЖДЕНИЕ РЕШЕНИЯ В ВИДЕ ПОСЛЕДОВАТЕЛЬНОСТИ ХОДОВ С ПОМОЩЬЮ ГРАФОВ
- (20 июн 25) graph_algs_8.py, код портирован с Object Pascal (Delphi) на Python. Взято из книги «Занимательное программирование. Самоучитель» 2005, страницы 145 - 175
...
## АЛГОРИТМ КРАСКАЛА
- заводится список всех стен, случайным образом из списка выбирается одна и сносится, если локации, к ней прилегающие не имеют прохода друг к другу
...
- улучшение волновой трассировки: на каждой итерации работы алгоритма происходит поиск локаций, помеченных числом N. Чтобы не пробегать по всему лабиринту, можно просто хранить их в отдельном списке (разумеется, его придётся постоянно обновлять)
...
- улучшение волновой трассировки: можно "разлить кисель" не только в стартовой локации, но и в финишной. Как только оба потока жидкости пересекутся в некоторой локации, маршрут найден.
  ...
  - стрелками теперь отмечается направление хода для каждого из двух киселей

# СРАВНЕНИЯ и АНАЛОГИИ
- Поиск в глубину похож на рекурсивный обход лабиринта, потому что заточен на определение тупиков и выхода из них. Лабиринт может в общем случае представлять собой циклический граф. Для предотвращения зацикливания внутри таких графов используется приём, когда новые вершины добавляются в конец списка обхода.
- Поиск в ширину похож на алгоритм волновой трассировки. Волновая трассировка на каждом шаге помечает локации, отстоящие на одном расстоянии от стартовой, то бишь, находящиеся на одном уровне глубины от стартовой локации.
